...
# Paket Brand
brand_standar = [
    {"nama": "1 Minggu", "harga": "40.000", "durasi": "1 Minggu"},
    {"nama": "1 Bulan", "harga": "100.000", "durasi": "1 Bulan"},
    {"nama": "2 Bulan", "harga": "140.000", "durasi": "2 Bulan"},
    {"nama": "6 Bulan", "harga": "400.000", "durasi": "6 Bulan"},
    {"nama": "1 Tahun", "harga": "900.000", "durasi": "1 Tahun"}
]

brand_premium = {
    "nama": "Premium 1 Tahun",
    "harga": "2.000.000",
    "durasi": "1 Tahun",
    "bonus": "Bonus Website Utama"
}

# Rate limiter
limiter = Limiter(get_remote_address, app=app, default_limits=["10000 per day", "10000 per hour"]) 
...
def init_db():
    ...
    file_type TEXT NOT NULL,       -- "pdf" atau "foto"
...
@app.route('/PaketMahasiswa')
def PaketMahasiswa():
    return render_template('Pricelist2.html')
...
# Halaman login khusus kerjasama
@app.route('/login-kerjasama', methods=['GET', 'POST'])
def login_kerjasama():
    if request.method == 'POST':
        email = request.form['email']
        password = request.form['password']

        # Cek di database user berdasarkan email
        user = db.execute("SELECT * FROM users WHERE email = ?", (email,)).fetchone()

        if user and check_password_hash(user['password'], password):
            if user['is_verified']:  # pastikan email sudah diverifikasi
                session['user_email'] = email  # simpan email di session
                return redirect(url_for('form_kerjasama'))
            else:
                flash("Email belum diverifikasi. Silakan cek inbox Anda.")
                return redirect(url_for('login_kerjasama'))
        else:
            flash("Email atau password salah.")
            return redirect(url_for('login_kerjasama'))
    return render_template('login-kerjasama.html')

@app.route('/form-kerjasama', methods=['GET', 'POST'])
def form_kerjasama():
    if 'user_email' not in session:
        return redirect(url_for('login_kerjasama'))
    if request.method == 'POST':
        paket_dipilih = request.form['paket']
        session['paket_dipilih'] = paket_dipilih
        return redirect(url_for('payment'))
    return render_template('form-kerjasama.html', 
                           brand_standar=brand_standar, 
                           brand_premium=brand_premium)
...
def payment():
    if 'user' not in session:
        return redirect(url_for('login_kerjasama'))
    paket = session.get('paket_dipilih')
    if request.method == 'POST':
        # Simulasi payment sukses
        return redirect(url_for('payment_sukses'))
    return render_template('payment.html', paket=paket)

# Halaman payment sukses
@app.route('/payment-sukses')
def payment_sukses():
    if 'user' not in session:
        return redirect(url_for('login_kerjasama'))
    paket = session.get('paket_dipilih')
    return render_template('payment-sukses.html', paket=paket)
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))  # Ambil port dari environment Render
    app.run(host="0.0.0.0", port=port)        # Listen di semua interface
